...
async function runInteractiveCLI() {
    ...
    const answers = await inquirer.prompt([
        ...
        },
        {
            type: 'input',
            name: 'keyLocation',
            message: chalk.yellow('Enter your IndexNow API key location:'),
            validate: (value) => {
                if (value.trim().length > 0) return true;
                return 'i.e. https://example.com/indexnow-key.txt';
            }
        },
...
program
    ...
    .option('-l, --key-location <url>', 'IndexNow API key location (i.e., https://example.com/<apiKey>.txt)')
